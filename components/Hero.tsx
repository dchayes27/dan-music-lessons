      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              <a 
                ...
                className="bg-sage text-white px-8 py-4 rounded-lg font-semibold hover:bg-sage/90 transition-all hover:shadow-lg hover:-translate-y-0.5"
              >
                Book Through JAM →
              </a>
              ...
              <a 
                href="#about"
                className="bg-white text-mauve border-2 border-mauve px-8 py-4 rounded-lg font-semibold hover:bg-mauve hover:text-white transition-all"
              >
                Learn More
              </a>
            </div>
          ...
          <div className="relative flex items-center justify-center min-h-[400px]">
            ...
            {/* Sampler grid - background right */}
            <div className="absolute bottom-0 right-0 grid grid-cols-4 gap-1 opacity-30">
              {Array.from({ length: 16 }).map((_, i) => (
                <div 
                  key={i} 
                  className={`w-5 h-5 rounded-sm ${i % 2 === 0 ? 'bg-mauve' : 'bg-honey'}`}
                />
              ))}
            </div>
            ...
            <div className="relative w-[280px] h-[320px]">
              {/* Strings */}
              <div className="absolute top-0 left-[120px] w-0.5 h-60 bg-slate" />
              <div className="absolute top-0 left-[130px] w-0.5 h-60 bg-slate" />
              <div className="absolute top-0 left-[140px] w-0.5 h-60 bg-slate" />
              <div className="absolute top-0 left-[150px] w-0.5 h-60 bg-slate" />
            ...
            <div className="absolute bottom-4 left-4 right-4 h-12 opacity-20">
              <div className="h-full flex items-end justify-around gap-0.5">
                {[3, 8, 5, 12, 7, 10, 4, 11, 6, 9, 5, 7, 12, 6, 8, 4].map((height, i) => (
                  <div 
                    key={i} 
                    className="w-1 bg-slate rounded-sm" 
